...
export function BookSessionModal({ isOpen, therapist, slots, onClose, onBooked }: Props) {
    const availableFutureSlots = useMemo(
        () =>
            slots
                .filter((slot) => slot.available && new Date(slot.start).getTime() > Date.now())
                .sort((a, b) => a.start.localeCompare(b.start)),
        [slots]
    );
    const byDay = useMemo(() => groupSlotsByDay(availableFutureSlots), [availableFutureSlots]);
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError((err as { message?: string }).message ?? 'Booking failed. Please try again.');
        } finally {
            setIsLoading(false);
        }
    };
